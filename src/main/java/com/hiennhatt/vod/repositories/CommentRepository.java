...
public interface CommentRepository extends JpaRepository<Comment, Integer> {
    ...
    @Query("SELECT c FROM Comment c WHERE c.video = :video and (c.status = 'ACTIVE' or c.user = :user) ORDER BY c.uid")
    List<CommentProjection> findCommentsInPublic(@NotNull Video video, @NotNull User user);
