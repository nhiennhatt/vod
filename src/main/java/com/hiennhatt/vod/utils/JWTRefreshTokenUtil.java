...
public class JWTRefreshTokenUtil {
    ...
    private RSAPublicKey getPublicKey() throws NoSuchAlgorithmException, InvalidKeySpecException {
        ...
        EncodedKeySpec keySpec = new X509EncodedKeySpec(Base64.getDecoder().decode(env.getProperty("jwt.keypair.refresh.publicKey")));
        return (RSAPublicKey) keyFactory.generatePublic(keySpec);
    }

    private RSAPrivateKey getPrivateKey() throws NoSuchAlgorithmException, InvalidKeySpecException {
        KeyFactory keyFactory = KeyFactory.getInstance("RSA");
        EncodedKeySpec keySpec = new PKCS8EncodedKeySpec(Base64.getDecoder().decode(env.getProperty("jwt.keypair.refresh.privateKey")));
